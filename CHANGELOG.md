...
### Fixed
- Context-aware completion token scaling to prevent MAX_TOKENS_TOO_HIGH errors
- Proactive eviction now accounts for completion token space requirements
- max_tokens calculation capped at context-appropriate size instead of always 4096

### Changed
- Added max_output_tokens field to Backend base class
- Implemented calculate_desired_completion_tokens() function that scales from 1,374 tokens (8K context) to 4,096 tokens (32K+ context)
