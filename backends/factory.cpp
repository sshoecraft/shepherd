...
std::unique_ptr<Backend> BackendFactory::create_backend(std::string &name, size_t context_size, Session& session, Backend::EventCallback callback) {
    ...
    dout(1) << "Creating backend: " + name + " with " + std::to_string(context_size) + " context_size" << std::endl;

    std::unique_ptr<Backend> backend;

    if (name == "llamacpp") {
#ifdef ENABLE_LLAMACPP
        backend = std::make_unique<LlamaCppBackend>(context_size, session, callback);
#else
...
#ifdef ENABLE_TENSORRT
        backend = std::make_unique<TensorRTBackend>(context_size, session, callback);
#else
        ...
        backend = std::make_unique<OpenAIBackend>(context_size, session, callback);
#else
...
#ifdef ENABLE_API_BACKENDS
        backend = std::make_unique<OllamaBackend>(context_size, session, callback);
#else
...
#ifdef ENABLE_API_BACKENDS
        backend = std::make_unique<AnthropicBackend>(context_size, session, callback);
#else
...
#ifdef ENABLE_API_BACKENDS
        backend = std::make_unique<GeminiBackend>(context_size, session, callback);
#else
        ...
        backend = std::make_unique<CLIClientBackend>(base_url, session, callback);
    }
    else {
        throw std::runtime_error("Unknown backend: " + name);
    }

    dout(1) << "Successfully created " + name + " backend" << std::endl;
    return backend;
}
