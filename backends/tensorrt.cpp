...
Response TensorRTBackend::add_message(Session& session, Message::Type type,
    ...
    if (message_tokens == 0) {
        if (type == Message::SYSTEM && !session.tools.empty()) {
            std::string formatted = format_system_message_with_tools(content, session.tools);
    ...
    if (!tokenize_and_accumulate_message(msg)) {
        Response err;
        err.success = false;
    ...
    if (type != Message::SYSTEM) {
        // Generate response
        std::string response_text = generate_internal(session, max_tokens);
        ...
        resp.finish_reason = tool_calls.empty() ? "stop" : "tool_calls";
...
Response TensorRTBackend::generate_from_session(const Session& session, int max_tokens) {
    ...
    if (new_messages > 0) {
        ...
        for (size_t i = matching_prefix; i < session.messages.size(); i++) {
            ...
            if (!tokenize_and_accumulate_message(msg_copy)) {
                Response err_resp;
                err_resp.success = false;
                err_resp.code = Response::ERROR;
    ...
    // Generate response
    std::string result = generate_internal(backend_session, max_tokens);
...
std::string TensorRTBackend::render_message(const Message& msg, bool add_generation_prompt) {
    ...
    try {
        return (*template_ptr)->render(context);
    } catch (const std::exception& e) {
        LOG_WARN("Exception rendering message: " + std::string(e.what()));
        return msg.content;
    }
...
bool TensorRTBackend::tokenize_and_accumulate_message(Message& msg) {
    if (!tokenizer_) {
        LOG_ERROR("Tokenizer not loaded");
        return false;
    }
    ...
    std::string formatted_text = render_message(msg, false);
...
std::string TensorRTBackend::format_system_message_with_tools(const std::string& system_content,
                                                               const std::vector<Session::Tool>& tools) {
    // For models that don't need special tool formatting
    if (model_config_.family != ModelFamily::QWEN_2_X &&
        model_config_.family != ModelFamily::LLAMA_3_X) {
        if (tools.empty()) {
    ...
    if (model_config_.family == ModelFamily::QWEN_2_X && template_node_) {
    ...
    // For Llama 3.x: Return unmodified (tools go in first user message)
    return system_content;
}

std::string TensorRTBackend::generate_internal(const Session& session, int max_tokens) {
    namespace tle = tensorrt_llm::executor;
    ...
    try {
        ...
        while (request_active_) {
            auto responses = executor->awaitResponses(request_id, std::chrono::milliseconds(100));

            for (const auto& response : responses) {
                if (response.hasError()) {
                    LOG_ERROR("Response error: " + response.getErrorMsg());
                    request_active_ = false;
                    throw BackendError("TensorRT generation failed: " + response.getErrorMsg());
                }
