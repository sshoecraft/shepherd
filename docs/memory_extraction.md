...
- **v2.34.0**: `--nomemory` flag to disable injection/extraction regardless of provider config. Per-request `"memory": false` in API server and CLI server request JSON. `FrontendFlags` struct replaces proliferating bool parameters. CLI client backend sends `"memory": false` to server when `--nomemory` is set.
- **v2.33.2**: Extraction input now contains only USER messages (assistant messages excluded). Prevents extraction model from storing assistant-generated content as user facts. Extraction prompt strengthened to reject transient data, operational state, and discussed content. RAG initialization deferred to provider connect — only initialized when provider has `memory: true`.
