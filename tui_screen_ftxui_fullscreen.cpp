...
bool TUIScreen::init() {
    ...
    // Enable mouse for scroll wheel capture
...
void TUIScreen::shutdown() {
    ...
    std::vector<ColoredLine> saved_lines;
    {
        ...
        if (!current_output_line.empty()) {
            saved_lines.push_back({current_output_line, current_output_color});
        }
    }
...
std::shared_ptr<fx::ComponentBase> TUIScreen::build_ui() {
    ...
    auto renderer = fx::Renderer(input_component, [this] {
        ...
        for (int i = start_idx; i < end_idx && i < static_cast<int>(output_lines.size()); i++) {
            ...
            // Indent model output (lines not starting with "> " or "* " or "  ")
            bool needs_indent = !line.text.empty() &&
        ...
        if (show_partial) {
            bool needs_indent = current_output_line.substr(0, 2) != "> " &&
                               current_output_line.substr(0, 2) != "* " &&
                               current_output_line.substr(0, 2) != "  ";
...
void TUIScreen::write_output(const char* text_data, size_t len, Color color) {
    {
        ...
        // Update color for current line
        current_output_color = color;
        ...
        for (size_t i = 0; i < len; i++) {
            ...
            if (c == '\n') {
                output_lines.push_back({current_output_line, current_output_color});
