...
bool TUIScreen::init() {
    // Use TerminalOutput mode
    screen = new fx::ScreenInteractive(fx::ScreenInteractive::TerminalOutput());

    // Disable mouse so terminal handles it natively (copy/paste works)
    screen->TrackMouse(false);

    // Set scroll region - reserve bottom 6 lines for FTXUI input area
    int rows = fx::Terminal::Size().dimy;
    int scroll_bottom = rows - 6;
    if (scroll_bottom < 1) scroll_bottom = rows - 1;
    printf("\033[1;%dr", scroll_bottom);  // Set scroll region
