...
// WebSearchManager implementation
WebSearchManager& WebSearchManager::instance() {
    static WebSearchManager manager;
    return manager;
}

void WebSearchManager::initialize(const std::string& provider, const std::string& api_key, const std::string& instance_url) {
    if (provider == "brave") {
        if (api_key.empty()) {
...
std::vector<WebSearchResult> WebSearchManager::search(const std::string& query) {
    if (!provider_) {
...
std::string WebSearchManager::get_provider_name() const {
    if (!provider_) {
        return "none";
    }
